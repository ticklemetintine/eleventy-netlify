...
var tgem2AssetPath = "/ed/t-gem-chart2/zh/";
// var tgem2AssetPath = "./";
...
chart2svg.append("line")
  ...
  .style("stroke", colorEquity)
  .attr("stroke-width", 3)
  .attr("x1", 350)     // x position of the first end of the line
  .attr("y1", height + 70)      // y position of the first end of the line
  .attr("x2", 390)     // x position of the second end of the line
  .attr("y2", height + 70);    // y position of the second end of the line
chart2svg.append("text")
  .text("全球股票投资组合")
  // .style("stroke", colorEquity)
...
function chart2ShowToolTipA() {
  chart2ShowToolTip({
    ...
    rectXCoor: chart2x(-61.5),
    rectYCoor: chart2y(35),
    width: 200,
    textColor: "black",
    dotted: false,
    outlineColor: colorBond,
    textValueLineA: "全球债券投资组合",
    textValueLineB: "曲线较窄,年度同比",
    textValueLineC: "回报波动性较低",
...
function chart2ShowTemToolTipB() {
  chart2ShowToolTipB({
    ...
    textValueLineB: "曲线较宽,年度同比",
    textValueLineC: "回报波动性较高",
...
function chart2ShowToolTip(params) {
  ...
  aToolTip.append("svg:image")
    ...
    .attr("x", params.rectXCoor+40)
    .attr("y", params.rectYCoor-4)
    .attr("height", 111)    // set the height
    .attr("width", 245)
    .attr("xlink:href", tgem2AssetPath + "chart2-callout1-zh.png")
    .attr("clip-path", "url(#" + params.maskClipId + ")");
  aToolTip
    .append("text")
    .attr("id", params.toolTipId + "TextA")
    .attr("x", params.rectXCoor + 52)
    .attr("y", params.rectYCoor + 21)
...
function chart2ShowToolTipB(params) {
  ...
  aToolTip.append("svg:image")
    ...
    .attr("y", params.rectYCoor+9)
    .attr("height", 161)    // set the height
    .attr("width", 187)
    .attr("xlink:href",tgem2AssetPath + "chart2-callout2-zh.png")
    .attr("clip-path", "url(#" + params.maskClipId + ")");
  aToolTip
    .append("text")
    .attr("id", params.toolTipId + "TextA")
    .attr("x", params.rectXCoor + 90)
    .attr("y", params.rectYCoor + 35)
    .text(params.textValueLineA)
    .style('fill', params.textColor)
    .style("text-anchor", "start")
    .attr("clip-path", "url(#" + params.maskClipId + ")");
...
$(document).ready(function(){
	$("<span class='tooltip' title='1. 全球股票投资组合采用MSCI ACWI全球指数作为参照标准。<br/>2. 全球债券投资组合采用彭博巴克莱全球综合指数 (Bloomberg Barclays Global Aggregate) 中的政府债券作为参照标准。' data-title='1. 全球股票投资组合采用MSCI ACWI全球指数作为参照标准。<br/>2. 全球债券投资组合采用彭博巴克莱全球综合指数 (Bloomberg Barclays Global Aggregate) 中的政府债券作为参照标准。'>图表注解</span>").insertAfter( $("#chart_tgem2") );
